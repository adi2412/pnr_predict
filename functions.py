...
from settings import DB_URL, DB, COLLECTION
...
def getPrediction(array_conditions, hours_before, wl):
    parameters = {
    ...
    data = getRelevantData(collection,array_conditions, sort_by, 20)
    result = prediction(hours_before, parameters["waiting_list"], data)
